...
def initialize_excited_species_metadata():
    for specie_name in specie_name_list:
        ...
        ensure_real_comp(cesium_excited_atom, "lifetime")
        ensure_real_comp(cesium_excited_atom, "age")
...
def dexcite_high_excited_atoms(step):
    ...
    write_status(
        ...
        step,
        f"targets={len(particle_buffer)} total_tiles={total_decay_tiles} total_decays={total_decay_particles}",
    )
    for target_name, data in particle_buffer.items():
        ...
        pc_target.add_particles(
            z=concatenate_chunks(data["z"], xp),
            ux=concatenate_chunks(data["ux"], xp),
            uy=concatenate_chunks(data["uy"], xp),
            uz=concatenate_chunks(data["uz"], xp),
            w=concatenate_chunks(data["w"], xp),
            lifetime=concatenate_chunks(data["lifetime"], xp),
            age=concatenate_chunks(data["age"], xp),
        )
